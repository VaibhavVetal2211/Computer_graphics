//DDA & BRESHLINE (DASH & DOT)

/*
Q3.
Write a Program in OpenGL on Linux Platform to draw a Dash-Dot-Dash Line
& Dash Line using DDA Line Drawing Algorithm. Divide the screen in four
quadrants with center as (0,0). Use Mouse / Without Mouse to Plot the Points.
*/
...
void ddaDashDotLine (float x1, float y1, float x2, float y2)
{
    ...
    for (int i = 0; i <= step; i++)
    {
        ...
        if (i%15 == 4 || i%15 == 5 || i%15 == 6 || i%15 == 7 || i%15 == 8 || i%15 == 9 || i%15 == 10 || i%15 == 11)
            displayPoint(x, y);			 //Draw the line segment (dash)
        else if (i%15 == 0)
            displayPoint(x, y);				 // Draw the dot
        x = x + xinc;
        y = y + yinc;
    }
}
void breshline_dash(float x1,float y1,float x2,float y2)
{
